...
void Line::clip(int xmin, int xmax, int ymin, int ymax)
{
	...
	{
		if (leftcode & 16) // if above just set y value to ymax
			lefty = ymax;

		if (rightcode & 16) // same as above if right endpoint above
			righty = ymax;

		if (leftcode & 8) // if below set y value to ymin
			lefty = ymin;

		if (rightcode & 8) // same as above if right endpoint below
			righty = ymin;
	...
	if (leftcode & 8) // clip below if left point
	{
	...
	if (rightcode & 8) // clip below if right point
	{
		x = int(round(leftx + (ymin - lefty) / m));
		rightx = x;
		righty = ymin;
	...
	if (leftcode & 16) // clip above if left point
	{
	...
	if (rightcode & 16) // clip above if right point
	{
		x = int(round(leftx + (ymax - lefty) / m));
		rightx = x;
		righty = ymax;
