...
void Line::ddaDraw(float* buffer, int width) // dda algorithm
{
	...
	if (leftx == rightx && lefty == righty) // same point
		return;
...
void Line::linInterpol(float* buffer, int width, float r1, float g1, float b1, float r2, float g2, float b2)
{
	float cr, cg, cb, dy, dx, tr1, tg1, tb1, tr2, tg2, tb2;
	...
	ColorPix(leftx, lefty, buffer, width, tr1, tg1, tb1); //turn on left endpoint
	...
	if (rightx == leftx) // vertical line cannot calculate m
	{
		...
		for (int i = 0; i <= abs(righty - lefty); i++)
		{
			cr = (tempy2 - (tempy + i)) / dy * tr1 + i / dy * tr2;
			cg = (tempy2 - (tempy + i)) / dy * tg1 + i / dy * tg2;
			cb = (tempy2 - (tempy + i)) / dy * tb1 + i / dy * tb2;
			ColorPix(leftx, tempy + i, buffer, width, cr, cg, cb);
		}
		return;
	}

	float m = float((righty - lefty)) / (rightx - leftx);
	float mbar = 1 / m;
	float x = leftx;
	float y = lefty;
	int roundedy = 0;
	...
	if (m == 0) // horizontal line
	{
		for (int i = 1; i <= rightx - leftx; i++)
		{
			cr = (rightx - (x + i)) / dx * tr1 + i / dx * tr2;
			cg = (rightx - (x + i)) / dx * tg1 + i / dx * tg2;
			cb = (rightx - (x + i)) / dx * tb1 + i / dx * tb2;
			ColorPix(int(x + i), int(y), buffer, width, cr, cg, cb);
		}
	}

	else if (m >= -1 && m <= 1) //shallow slope
	{
		for (int i = 1; i <= rightx - leftx; i++)
		{
			y = y + m;
			...
			cg = (rightx - (x + i)) / dx * tg1 + i / dx * tg2;
			cb = (rightx - (x + i)) / dx * tb1 + i / dx * tb2;
			ColorPix(int(x+i), roundedy, buffer, width, cr, cg, cb);
		}
	}
	...
	else if (m > 1) //steep postive slope
	{
		for (int i = 1; i <= righty - lefty; i++)
		{
			x = x + mbar;
			roundedx = int(round(x));
			cr = (righty - (y + i)) / dy * tr1 + i / dy * tr2;
			cg = (righty - (y + i)) / dy * tg1 + i / dy * tg2;
			cb = (righty - (y + i)) / dy * tb1 + i / dy * tb2;
			ColorPix(roundedx, int(y+i), buffer, width, cr, cg, cb);
		}
	}
